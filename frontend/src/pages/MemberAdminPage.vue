...
<template>
  <section class="mx-auto grid min-w-0 max-w-6xl gap-4 overflow-x-hidden p-3 text-stone-900 dark:text-stone-100 sm:p-4">
	...
	<div v-if="paymentDetail" class="fixed inset-0 z-[60] grid place-items-end bg-black/55 p-3 sm:place-items-center" role="dialog" aria-modal="true" aria-label="รายละเอียดการชำระเงิน" @click.self="paymentDetail=null">
	  <section class="max-h-[88dvh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-4 shadow-2xl dark:bg-stone-900">
		...
		<div class="mt-4 divide-y rounded-lg border dark:border-stone-700 dark:divide-stone-700">
		...
		<div class="mt-4 flex items-center justify-between rounded-lg bg-paper-100 p-3 dark:bg-stone-800"><b>ยอดชำระรวม</b><b class="text-lg text-court-700">฿{{ Number(paymentDetail.amountThb || 0).toLocaleString('th-TH') }}</b></div>
	  </section>
	</div>
